...
int main() {
    ...
    { // Serializing
        { // Direct Vulkan with macro
            ...
            bool parsed = VK_SERIALIZE(VkImageLayout, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, &serialized);
            // serialized will now be `READ_ONLY_OPTIMAL`
        }

        { // Using template function directly
            std::string serialized;
            bool parsed = vk_serialize<VkImageLayout>("VkImageLayout", VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, &serialized);
            // serialized will now be `READ_ONLY_OPTIMAL`
        }

         { // vk_parse directly, converting against common uint32_t base type
            std::string serialized;
            bool parsed = vk_serialize("VkImageLayout", static_cast<uint32_t>(VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL), &serialized);
