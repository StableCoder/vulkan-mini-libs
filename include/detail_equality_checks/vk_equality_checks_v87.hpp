...
#ifndef VK_EQUALITY_CHECK_V87_HPP
#define VK_EQUALITY_CHECK_V87_HPP
...
static_assert(VK_HEADER_VERSION == 87, "Incompatible VK_HEADER_VERSION!" );
...
bool operator==(VkShaderModuleCreateInfo const &lhs,
                VkShaderModuleCreateInfo const &rhs) noexcept {
  if(lhs.latexmath:[codeSize \over 4] != rhs.latexmath:[codeSize \over 4])
    return false;

  for(int i = 0; i < lhs.latexmath:[codeSize \over 4]; ++i) {
...
bool operator==(VkCmdProcessCommandsInfoNVX const &lhs,
  ...
  for(int i = 0; i < lhs.indirectCommandsTokenCount; ++i) {
...
bool operator==(VkPhysicalDeviceDriverPropertiesKHR const &lhs,
                VkPhysicalDeviceDriverPropertiesKHR const &rhs) noexcept {
  for(int i = 0; i < VK_MAX_DRIVER_NAME_SIZE_KHR; ++i) {
    if(lhs.driverName[i] != rhs.driverName[i])
      return false;
  }

  for(int i = 0; i < VK_MAX_DRIVER_INFO_SIZE_KHR; ++i) {
...
bool operator==(VkRaytracingPipelineCreateInfoNVX const &lhs,
  ...
  for(int i = 0; i < lhs.stageCount; ++i) {
    if(lhs.pGroupNumbers[i] != rhs.pGroupNumbers[i])
      return false;
  }
...
bool operator==(VkPhysicalDeviceRaytracingPropertiesNVX const &lhs,
                VkPhysicalDeviceRaytracingPropertiesNVX const &rhs) noexcept {
  return (lhs.sType == rhs.sType) &&
         (lhs.shaderHeaderSize == rhs.shaderHeaderSize) &&
         (lhs.maxRecursionDepth == rhs.maxRecursionDepth) &&
...
#endif // VK_EQUALITY_CHECK_V87_HPP
